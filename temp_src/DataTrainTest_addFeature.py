...
# --- 설정 변수 ---
# 모델 및 Load 파일 저장 폴더
TRAINED_DATA_DIR = '/home/kanghun/수업_AXI/기말고사/1_Korean_Apartment_Deal/trained_data'
...
PLOT_OUTPUT_DIR = '/home/kanghun/수업_AXI/기말고사/1_Korean_Apartment_Deal/results'  # 그래프 및 추천 결과 저장 폴더
...
def plot_apartment_timeseries(unique_id, original_df, reco_df, model, base_date, features, is_best=False):
    ...
    # ✅ [추가] 매크로 피처를 base_row 값으로 고정해서 사용
    for col in MACRO_FEATURES:
        if col in base_row:
            encoded_values[col] = base_row[col]
    ...
    future_X = pd.DataFrame(future_data)[features]  # ✅ 매크로 포함된 features 사용
...
data_csv_path = prepare_csv_from_zip(data_dir, 'KoreaApartDeal_PreProcessed2.csv', 'KoreaApartDeal_PreProcessed2.zip')
...
if SHOULD_RETRAIN:
    ...
    # ✅[추가] 거시경제 / 인구 피처 숫자형 변환
    for col in MACRO_FEATURES:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce')

    # ✅ [추가] core_features에 매크로 피처 포함
    core_features = ['거래일', '건축년도', '전용면적', '거래금액', '시도명', '시군구명', '법정동', '아파트'] + MACRO_FEATURES 
...
features = ['시도명', '시군구명', '법정동', '아파트', '전용면적',
            '건축년도', '거래_년', '거래_월', '건축_경과년수', '최근_거래일_점수'] + MACRO_FEATURES  # 층을 빼고 예측에 사용 (UniqueID에 포함됨)
target = '거래금액'
...
# ✅ [수정] 매크로 피처도 포함
base_cols = ['UniqueID', '시도명', '시군구명', '법정동', '아파트', '전용면적', '건축년도'] + MACRO_FEATURES
...
# ✅ [추가] 추천 결과에 매크로 피처도 포함
reco_df = all_unique_apts[['UniqueID', '시도명', '시군구명', '법정동', '아파트', '전용면적', '건축년도']+ MACRO_FEATURES].copy()
